...
class Plugin extends AbstractPlugin implements LoopAwareInterface
{
    ...
    /**
     * @var \Infonomicon\IrcBot\WordGame\WordProvider
     */
    private $wordProvider;

    /**
     * @var \Infonomicon\IrcBot\WordGame\Scoreboard
     */
    private $scoreboard;

    /**
     * @var \Infonomicon\IrcBot\WordGame\Word
     */
    private $word;
    ...
    /**
     * The config array must have:
     *   'word_provider' => instanceof \Infonomicon\IrcBot\WordGame\WordProvider
     *   'scoreboard'    => instanceof \Infonomicon\IrcBot\WordGame\Scoreboard
     *
     * @param array $config
     */
    public function __construct(array $config)
    {
        if (!isset($config['word_provider']) || !($config['word_provider'] instanceof WordProvider)) {
            throw new \InvalidArgumentException('Invalid word provider configured');
        }

        if (!isset($config['scoreboard']) || !($config['scoreboard'] instanceof Scoreboard)) {
            throw new \InvalidArgumentException('Invalid scoreboard configured');
        }

        $this->wordProvider = $config['word_provider'];
